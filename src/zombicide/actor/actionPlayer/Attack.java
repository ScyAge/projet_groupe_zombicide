...
public class Attack implements ActionsPlayer{
	...
	public List<Zombies> WhoCanAttack(Player player,Weapon w){
		...
		for(Direction D : Direction.values()) {
			...
			i=0;
			if( D == Direction.West) {
				while(c.getDoor(D).isBreak() && c.getY()-1 - i>0 && i < w.getRange()-1 &&i>=w.getMinRange()&&this.board.getCellBoard(c.getX(), c.getY()-1).getDoor(Direction.East).isBreak()) {
			...
			if( D == Direction.East) {
				while(c.getDoor(D).isBreak() && c.getY()+i+1< this.board.getBoard()[0].length && i < w.getRange()-1&&i>=w.getMinRange()&&this.board.getCellBoard(c.getX(), c.getY()+1).getDoor(Direction.West).isBreak()) {
					c = this.board.getCellBoard(c.getX(), c.getY()+1);
					z.addAll(c.getAllZombies());
					i+=1;
				}
			...
			if( D == Direction.South) {
				while(c.getDoor(D).isBreak() && i+c.getX()+1< this.board.getBoard().length &&i < w.getRange()-1&&i>=w.getMinRange()&&this.board.getCellBoard(c.getX()+1, c.getY()).getDoor(Direction.North).isBreak()) {
			...
			if( D == Direction.North) {
				while(c.getDoor(D).isBreak() && c.getX()-i-1> 0 && i < w.getRange()-1&&i>=w.getMinRange()&&this.board.getCellBoard(c.getX()-1, c.getY()).getDoor(Direction.South).isBreak()) {
					c = this.board.getCellBoard(c.getX()-1, c.getY());
					z.addAll(c.getAllZombies());
					i+=1;
				}
